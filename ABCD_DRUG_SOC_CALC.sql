/*
The rows count for the base table which holds the relation between Drug name and SOC is 45528187
*/

-- Create statement for temp table which calculate "A" value
-- The count of the rows 269117
create table soc_prodai_case_Final_a TABLESPACE nother_space as
(
	select
		prod_ai,
		soc_code,
...
-- The count of the rows 28
create table soc_sum_prodai_case_Final_g1 TABLESPACE nother_space as
(
	select
		soc_code,
		count(*) as g1
...
create table soc_prodai_case_Final_abcd_1 TABLESPACE nother_space as
(
	select 
	base.prod_ai,
	base.soc_code,
	...
	from soc_prodai_case_Final_a				as base
	left join prodai_case_Final_sum			as d_s
		on base.prod_ai = d_s.prod_ai
	left join soc_sum_prodai_case_Final_g1 			as ss
		on base.soc_code = ss.soc_code 
);

/*
Some quality and sanity checks on the ABCD table
*/
select * from soc_prodai_case_Final_abcd_1
where verify = 'False';
--returns 0
